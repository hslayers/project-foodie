...
module.controller('Main', ['$scope', 'Core', 'hs.query.baseService', 'hs.compositions.service_parser', '$timeout', 'hs.map.service', '$http', 'config', '$rootScope', 'hs.utils.service', '$compile', 'hs.query.wmsService', '$sce', 'gettext',
    function ($scope, Core, QueryService, composition_parser, $timeout, hsMap, $http, config, $rootScope, utils, $compile, WmsService, $sce, gettext) {
        ...
        function describeZone(zone, callback, links_to_id) {
            ...
            $scope.zone.special_attributes.push({
                short_name: 'Management zone',
                value: zone.properties.zone_name.getValue()
            });
            $scope.zone.special_attributes.push({
                short_name: 'Crop',
                value: zone.properties['crop description'].getValue()
            });
            $scope.zone.special_attributes.push({
                short_name: 'Production amount',
                value: zone.properties.amount.getValue()
            });
            $scope.zone.special_attributes.push({
                short_name: 'Campaign',
                value: moment(zone.properties.campaign_begin.getValue()).format('MMMM Do YYYY') + ' - ' + moment(zone.properties.campaign_end.getValue()).format('MMMM Do YYYY')
            });
            $.ajax({
                url: q
            })
